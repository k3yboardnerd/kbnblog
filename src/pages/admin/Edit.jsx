...
import { useCreateNewPostMutation } from "../../features/admin/posts/postsApiSlice"
...
export default function Edit() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!(title || category || description || cover)) {
      ...
      try {
        const res = await createPost({ user: userInfo._id, title, category, body: description, cover }).unwrap()
  ...
  if (userInfo) {
    if (userInfo.admin === true) {
      return (
        <div className="grid grid-cols-2 overflow-y-scroll scroll-m-0 overflow-x-hidden col-span-5 h-full place-content-start justify-between p-4 pb-40 gap-4">
          <article className="flex flex-col article content h-full max-h-screen p-2 border-2 shadow-2xl rounded-lg overflow-y-visible" dangerouslySetInnerHTML={{ __html: description ? description : "<h1 class='preview-title'>Post preview</h1>" }} />
          <form onSubmit={handleSubmit} className="flex flex-col space-y-4 min-h-full p-4" action="#" method="post">
            ...
            <div className="flex flex-col space-y-4">
              ...
              <section className="flex h-[450px] max-h-full w-full self-center my-2 hover:ring-violet-600 focus:ring-violet-800">
                <ReactQuill id="message" className="w-full h-full border-none rounded-lg p-2" modules={modules} value={description} onChange={setDescription} />
              </section>
              {/* <textarea value={description} onChange={(e) => setDescription(e.target.value)} required className="ring-2 ring-violet-400 rounded-md p-2 w-[98%] self-center my-2 resize-y hover:ring-violet-600 focus:ring-violet-800" name="message" id="message" cols="30" rows="4"></textarea> */}
            </div>
          </form>
        </div>
      )
